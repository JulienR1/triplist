<script lang="ts">
    ...
    import { itemIsValid, itemsAreEqual, arraysAreEqual } from "common";
    ...
    const handleItemChange = async () => {
        if (itemIsValid(item)) {
            ...
            if (itemsAreEqual(item, updatedItem)) {
                ...
                Toast.error(
                    "Une erreur est survenue au cours de la modification.",
                );
            }
        }
    };
    ...
    const handleToggle = async (event: Event) => {
        if (disabled) {
            Toast.info(
                "Les modifications sont désactivées lorsqu'un filtre est actif.",
            );
            return;
        }
        ...
        if (itemIsValid(item)) {
            ...
            if (!arraysAreEqual(savedChecks, item.values)) {
                if (savedChecks) {
                    ...
                    Toast.error(
                        "Erreur, la saisie n'a pas été prise en compte.",
                    );
                }
            }
        }
    ...
    <td>
        <DeleteWrapper toRight {disabled} on:click={handleDelete}>
            <ConfirmedEditableText
                ...
                on:datachange={handleItemChange}
            />
        </DeleteWrapper>
    </td>
    {#each item.values as checked}
        <td>
            <label
                ...
                class:checked
                tabindex="0"
                on:keypress|preventDefault={(event) => onKeyPress(event)}
            >
                <input
                    type="checkbox"
                    {disabled}
