<script lang="ts">
    ...
    onMount(() => {
        const unsubscribe = triplistData.subscribe(({ apiData }) => {
            if (apiData) {
                const activityLabels = apiData.activities.map((activity) => activity.label);
                const categoryLabels = apiData.categories.map((category) => category.label);
                const allOptions = [...new Set([...activityLabels, ...categoryLabels])];

                appliedFilters.forEach((filter) => {
                    const indexToRemove = allOptions.findIndex((option) => option === filter);
                    if (indexToRemove >= 0) {
                        allOptions.splice(indexToRemove, 1);
                    }
                });
