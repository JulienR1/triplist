...
const getItemsForCategory = async (activityFilters: string[], { id }: ICategory): Promise<IItem[] | undefined> => {
	if (positiveIntegerIsValid(id)) {
		...
		await DatabaseHandler.useConnection(async (connection) => {
			...
			const activityIds = rawActivityIds.map((rawId: { id: number }) => rawId.id);

			const checksParams = hasFilters ? [id, ...activityIds] : [id];
			const checksQuery = `SELECT * FROM checked_items WHERE category_id = ? ${
				hasFilters ? `AND checked = TRUE AND activity_id IN (${Array(activityIds.length).fill("?").join(",")})` : ""
			}`;
			...
			if (rawItems && rawChecks) {
				...
				items = storedItems.map((item: IItem) => ({
					...
					values: activityIds.map(
						(activityId: number) =>
							new Boolean(
								checks.find((check) => check.activity_id === activityId && check.item_id === item.id)?.checked || false
							)
					),
				}));
			}
...
const addItemToCategory = async (itemName: string, { id }: ICategory): Promise<IItem | undefined> => {
	...
	await DatabaseHandler.useConnection(async (connection) => {
		...
		const [rows] = (await connection.execute("SELECT * FROM item WHERE label = ? AND category_id = ?", [
			itemName,
			id,
		])) as RowDataPacket[];
...
const checkItem = async (item: IItem): Promise<boolean[]> => {
	...
	await DatabaseHandler.useConnection(async (connection) => {
		const [checkRows] = (await connection.execute("SELECT * FROM checked_items WHERE item_id = ?", [
			item.id,
		])) as RowDataPacket[];
		...
		const promises = item.values
			.map((value, index) => {
				if (value !== checks[index].checked) {
					...
					if (value) {
						return connection.execute("INSERT INTO activityitem (item_id, activity_id) VALUES (?, ?)", [
							item_id,
							activity_id,
						]);
					} else {
						return connection.execute("DELETE FROM activityitem WHERE item_id = ? AND activity_id = ?", [
							item_id,
							activity_id,
						]);
					}
		...
		const [rawStoredChecks] = (await connection.execute("SELECT * FROM checked_items WHERE item_id = ?", [
			item.id,
		])) as RowDataPacket[];
